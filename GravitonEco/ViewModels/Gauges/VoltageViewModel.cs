    ...
    public partial class VoltageViewModel : ObservableObject
    {
        ...
        private async Task PollRegistersAsync()
        {
            try
            {
                var tasks = new[]
                {
            Task.Run(() => _modbusTcpCommunication.ReadInputRegisters(1, 2)), // CurrentValue
            Task.Run(() => _modbusTcpCommunication.ReadHoldingRegisters(1, 8)), // Porog1
            Task.Run(() => _modbusTcpCommunication.ReadHoldingRegisters(1, 9)), // Porog2
            Task.Run(() => _modbusTcpCommunication.ReadHoldingRegisters(1, 10)), // Increment
            Task.Run(() => _modbusTcpCommunication.ReadHoldingRegisters(1, 11)), // Period
            Task.Run(() => _modbusTcpCommunication.ReadDiscreteRegisters(1, 6)), // AlarmPorog1
            Task.Run(() => _modbusTcpCommunication.ReadDiscreteRegisters(1, 7)), // AlarmPorog2
            Task.Run(() => _modbusTcpCommunication.ReadDiscreteRegisters(1, 8)), // AlarmPorog3
        };
