    ...
    public partial class CO2ViewModel : ObservableObject
    {
        ...
        private async Task PollRegistersAsync()
        {
            try
            {
                var tasks = new[]
                {
            Task.Run(() => _modbusTcpCommunication.ReadInputRegisters(1, 11)), // CurrentValue
            Task.Run(() => _modbusTcpCommunication.ReadHoldingRegisters(1, 44)), // Porog1
            Task.Run(() => _modbusTcpCommunication.ReadHoldingRegisters(1, 45)), // Porog2
            Task.Run(() => _modbusTcpCommunication.ReadHoldingRegisters(1, 46)), // Increment
            Task.Run(() => _modbusTcpCommunication.ReadHoldingRegisters(1, 47)), // Period
            Task.Run(() => _modbusTcpCommunication.ReadDiscreteRegisters(1, 33)), // AlarmPorog1
            Task.Run(() => _modbusTcpCommunication.ReadDiscreteRegisters(1, 34)), // AlarmPorog2
            Task.Run(() => _modbusTcpCommunication.ReadDiscreteRegisters(1, 35)), // AlarmPorog3
        };
