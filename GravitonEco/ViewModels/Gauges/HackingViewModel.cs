    ...
    public partial class HackingViewModel : ObservableObject
    {
        ...
        private async Task PollRegistersAsync()
        {
            try
            {
                var tasks = new[]
                {
            Task.Run(() => _modbusTcpCommunication.ReadInputRegisters(3, 2)), // CurrentValue
            Task.Run(() => _modbusTcpCommunication.ReadHoldingRegisters(3, 8)), // Porog1
            Task.Run(() => _modbusTcpCommunication.ReadHoldingRegisters(3, 9)), // Porog2
            Task.Run(() => _modbusTcpCommunication.ReadHoldingRegisters(3, 10)), // Increment
            Task.Run(() => _modbusTcpCommunication.ReadHoldingRegisters(3, 11)), // Period
            Task.Run(() => _modbusTcpCommunication.ReadDiscreteRegisters(3, 6)), // AlarmPorog1
            Task.Run(() => _modbusTcpCommunication.ReadDiscreteRegisters(3, 7)), // AlarmPorog2
            Task.Run(() => _modbusTcpCommunication.ReadDiscreteRegisters(3, 8)), // AlarmPorog3
        };
