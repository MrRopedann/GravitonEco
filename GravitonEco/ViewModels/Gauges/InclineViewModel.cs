    ...
    public partial class InclineViewModel : ObservableObject
    {
        ...
        private async Task PollRegistersAsync()
        {
            try
            {
                var tasks = new[]
                {
            Task.Run(() => _modbusTcpCommunication.ReadInputRegisters(1, 20)), // CurrentValue
            Task.Run(() => _modbusTcpCommunication.ReadHoldingRegisters(1, 80)), // Porog1
            Task.Run(() => _modbusTcpCommunication.ReadHoldingRegisters(1, 81)), // Porog2
            Task.Run(() => _modbusTcpCommunication.ReadHoldingRegisters(1, 82)), // Increment
            Task.Run(() => _modbusTcpCommunication.ReadHoldingRegisters(1, 83)), // Period
            Task.Run(() => _modbusTcpCommunication.ReadDiscreteRegisters(1, 60)), // AlarmPorog1
            Task.Run(() => _modbusTcpCommunication.ReadDiscreteRegisters(1, 61)), // AlarmPorog2
            Task.Run(() => _modbusTcpCommunication.ReadDiscreteRegisters(1, 62)), // AlarmPorog3
        };
