    ...
    public partial class InternalRelativeHumidityViewModel : ObservableObject
    {
        ...
        private async Task PollRegistersAsync()
        {
            try
            {
                var tasks = new[]
                {
            Task.Run(() => _modbusTcpCommunication.ReadInputRegisters(1, 6)), // CurrentValue
            Task.Run(() => _modbusTcpCommunication.ReadHoldingRegisters(1, 24)), // Porog1
            Task.Run(() => _modbusTcpCommunication.ReadHoldingRegisters(1, 25)), // Porog2
            Task.Run(() => _modbusTcpCommunication.ReadHoldingRegisters(1, 26)), // Increment
            Task.Run(() => _modbusTcpCommunication.ReadHoldingRegisters(1, 27)), // Period
            Task.Run(() => _modbusTcpCommunication.ReadDiscreteRegisters(1, 18)), // AlarmPorog1
            Task.Run(() => _modbusTcpCommunication.ReadDiscreteRegisters(1, 19)), // AlarmPorog2
            Task.Run(() => _modbusTcpCommunication.ReadDiscreteRegisters(1, 20)), // AlarmPorog3
        };
