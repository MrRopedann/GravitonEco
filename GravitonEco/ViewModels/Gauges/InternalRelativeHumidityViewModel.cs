...
{
    public partial class InternalRelativeHumidityViewModel : ObservableObject
    {
        ...
        public InternalRelativeHumidityViewModel()
        {
        ...
        {
            Porog1 = (await _modbusTcpCommunication.ReadHoldingRegistersAsync(1, 24))[0];
            Porog2 = (await _modbusTcpCommunication.ReadHoldingRegistersAsync(1, 25))[0];
            Increment = (await _modbusTcpCommunication.ReadHoldingRegistersAsync(1, 26))[0];
            Period = (await _modbusTcpCommunication.ReadHoldingRegistersAsync(1, 27))[0];
        }

        private async void WritePorog1Value()
        {
            await _modbusTcpCommunication.WriteSingleHoldingRegisterAsync(1, 24, Porog1);
        }

        private async void WritePorog2Value()
        {
            await _modbusTcpCommunication.WriteSingleHoldingRegisterAsync(1, 25, Porog2);
        }

        private async void WriteIncrementValue()
        {
            await _modbusTcpCommunication.WriteSingleHoldingRegisterAsync(1, 26, Increment);
        }

        private async void WritePeriodValue()
        {
            await _modbusTcpCommunication.WriteSingleHoldingRegisterAsync(1, 27, Period);
        }
        ...
        private async Task PollRegistersAsync()
        {
            try
            {
                var currentValueTask = Task.Run(() => _modbusTcpCommunication.ReadInputRegistersAsync(1, 6)); // CurrentValue
                var alarmPorog1Task = Task.Run(() => _modbusTcpCommunication.ReadDiscreteRegistersAsync(1, 18)); // AlarmPorog1
                var alarmPorog2Task = Task.Run(() => _modbusTcpCommunication.ReadDiscreteRegistersAsync(1, 19)); // AlarmPorog2
                var alarmPorog3Task = Task.Run(() => _modbusTcpCommunication.ReadDiscreteRegistersAsync(1, 20)); // AlarmPorog3
