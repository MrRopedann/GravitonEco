...
namespace GravitonEco.ViewModels.Gauges
{
    public partial class AtmosphericPressureViewModel : ObservableObject
    {
        ...
        {
            await _modbusTcpCommunication.WriteSingleHoldingRegisterAsync(1, 4, Porog1);
        }

        private async void WritePorog2Value()
        {
            await _modbusTcpCommunication.WriteSingleHoldingRegisterAsync(1, 5, Porog2);
        }

        private async void WriteIncrementValue()
        {
            await _modbusTcpCommunication.WriteSingleHoldingRegisterAsync(1, 6, Increment);
        }

        private async void WritePeriodValue()
        {
            await _modbusTcpCommunication.WriteSingleHoldingRegisterAsync(1, 7, Period);
        }
