    ...
    public partial class LOCViewModel : ObservableObject
    {
        ...
        private async Task PollRegistersAsync()
        {
            try
            {
                var tasks = new[]
                {
            Task.Run(() => _modbusTcpCommunication.ReadInputRegisters(1, 8)), // CurrentValue
            Task.Run(() => _modbusTcpCommunication.ReadHoldingRegisters(1, 32)), // Porog1
            Task.Run(() => _modbusTcpCommunication.ReadHoldingRegisters(1, 33)), // Porog2
            Task.Run(() => _modbusTcpCommunication.ReadHoldingRegisters(1, 34)), // Increment
            Task.Run(() => _modbusTcpCommunication.ReadHoldingRegisters(1, 35)), // Period
            Task.Run(() => _modbusTcpCommunication.ReadDiscreteRegisters(1, 24)), // AlarmPorog1
            Task.Run(() => _modbusTcpCommunication.ReadDiscreteRegisters(1, 25)), // AlarmPorog2
            Task.Run(() => _modbusTcpCommunication.ReadDiscreteRegisters(1, 26)), // AlarmPorog3
        };
